...
url = "https://www.emploitic.com/offres-d-emploi?q=java"
...
final_df.apply(lambda row:folium.Marker(location=[row["latitude"], row["longitude"]],
                                              radius=10, tooltip = len(re.findall("<li>", row["link"])) + "offers", popup="<b>"+row["location"]+":"+"<b/>"+row["link"], icon=folium.Icon(color='red', icon='fa-map-pin', prefix='fa'))
                                             .add_to(m), axis=1)
output_file = "map_fol.html"
m.save(output_file)
